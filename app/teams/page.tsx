...
export default function TeamsPage() {
  ...
  const [isBusinessTier, setIsBusinessTier] = useState(true); // optimistic; confirmed in fetchTeams
  ...
  const fetchTeams = async () => {
    ...
    try {
      ...
      // Tier gate — dual-table resilient read
      ...
      const resolvedTier = pickHighestTier(prof?.tier, ut?.tier);
      const isBiz = resolvedTier === 'business' || resolvedTier === 'business_plus';
      setIsBusinessTier(isBiz);
      if (!isBiz) { setLoading(false); return; }
      ...
      const teamIds = (data || []).map((m: any) => m.teams.id);
      const countMap: Record<string, number> = {};
      if (teamIds.length > 0) {
        const { data: allMembers } = await supabaseBrowser
          .from('team_members')
          .select('team_id')
          .in('team_id', teamIds);
      ...
      setTeams(
        (data || []).map((m: any) => ({
          ...
          name: m.teams.name,
          role: m.role,
          member_count: countMap[m.teams.id] || 1,
        }))
      );
    } catch {
      toast.error('Failed to load teams');
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        {!loading && !isBusinessTier ? (
          <div className="text-center py-20 border border-dashed border-white/10 rounded-2xl bg-card/60">
            <Users className="mx-auto h-12 w-12 text-muted-foreground/40 mb-4" />
            <h3 className="text-lg font-semibold mb-2">Team features require a business plan</h3>
            <p className="text-muted-foreground mb-6 max-w-sm mx-auto">
              Upgrade to Small Business or Business Plus to create teams and collaborate with your employees.
            </p>
            <Link href="/pricing">
              <Button className="btn-premium">View Business Plans</Button>
            </Link>
          </div>
        ) : (
          <>
              ...
              >
                {teams.map((team) => (
                  <motion.div key={team.id} variants={fadeInUp}>
                    <Card className="card-premium border-white/10 hover:border-primary/40 transition-colors h-full">
                      <CardHeader>
            ...
            <div className="mt-10 text-sm text-slate-500">
              Team features are available on <span className="font-medium text-slate-300">Small Business</span> and <span className="font-medium text-slate-300">Business Plus</span> plans.
              <Link href="/pricing" className="text-blue-400 hover:text-blue-300 hover:underline ml-1">Upgrade here</Link>
            </div>
          </>
        )}
      </div>
